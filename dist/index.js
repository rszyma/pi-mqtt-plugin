...
export default function homeAssistantMqttExtension(pi, customConfig) {
    let config = null;
    ...
    pi.on("session_start", async (_event, ctx) => {
        ...
        if (config.slot_overflow) {
            const msg = `No free MQTT slot: suffix ${config.instance_suffix} exceeds slot_count ${config.slot_count}. ` +
    ...
    pi.registerCommand("mqtt", {
        ...
        handler: async (args, ctx) => {
            ...
            if (sub === "reload") {
                ...
                ctx.ui.notify(loadError
                    ? `Reloaded settings (with error: ${loadError}) — run /reload to reconnect MQTT`
                    : "Reloaded MQTT settings — run /reload to reconnect with new config", loadError ? "warning" : "info");
                return;
            }
            if (sub === "edit") {
                const scope = (tokens[1] || "").toLowerCase();
                const target = scope === "global" ? "global" : "project";
                const settingsPath = target === "project" ? projectSettingsPath : globalSettingsPath;
